...
┌─────────────────────────────────────────────────────────────┐
│                     Admin UI (Port 8088)                     │
│  /users         → User CRUD, budget, permissions, API keys  │
│  /user/*        → User portal (login, dashboard, usage)     │
│  database.py    → PostgreSQL CRUD + Valkey sync                  │
└──────────────────────────┬──────────────────────────────────┘
                           │ shared Docker volume
                    /opt/moe-infra/userdb/users.db (PostgreSQL)
                           │
┌──────────────────────────┴──────────────────────────────────┐
│               Orchestrator (Port 8002) – main.py             │
│  _validate_api_key()    → Valkey lookup (5min TTL)           │
│  _log_usage_to_db()     → async PostgreSQL write (fire&forget)  │
│  _increment_user_budget() → Valkey counter                   │
│  PROM_TOKENS / PROM_REQUESTS → user_id label               │
└──────────────────────────┬──────────────────────────────────┘
                           │
               Valkey (terra_cache:6379)
               ├── user:apikey:{sha256}   HASH  TTL 300s
               ├── user:{id}:tokens:daily:{date}
               ├── user:{id}:tokens:monthly:{YYYY-MM}
               └── user:{id}:tokens:total
```
...
File: `/opt/moe-infra/userdb/users.db` (PostgreSQL, WAL mode)
...
_validate_api_key(raw_key)
    ...
    ├── Valkey HGETALL user:apikey:{key_hash}
    │     ├── Cache miss → return None (401)
    │     ├── is_active != "1" → return None (401)
    │     └── Budget check:
    │           ├── daily_limit set?
    │           │     Valkey GET user:{id}:tokens:daily:{date}
    │           │     ≥ limit → PROM_BUDGET_EXCEEDED.inc() + return None (429)
    │           ├── monthly_limit set? → analogous
    │           └── total_limit set? → analogous
    │
    └── return {user_id, username, permissions_json, ...}

Pipeline executed (AgentState.user_id set)
    │
    ▼
Post-request (fire-and-forget asyncio.create_task)
    ├── _log_usage_to_db() → PostgreSQL usage_log INSERT
    └── _increment_user_budget() → Valkey INCRBY (daily, monthly, total)
```
...
# Rebuild orchestrator (aiosqlite in requirements)
...
5. **PostgreSQL WAL mode**: Enables concurrent reads between two Docker containers without locks
