...
| `graph_rag` | Neo4j knowledge graph | **CAG gate** (v2.5): compliance-keyword queries (BAIT/VAIT/DORA/KRITIS) bypass Neo4j and inject pre-loaded authoritative text (Chan et al. 2024). Then: **3-stage retrieval pipeline**: (1) term-matching on original query; (2) **Query Reformulation** (v2.5): if step 1 empty, LLM generates 2 alternative phrasings and retries; (3) **Text-to-Cypher** (v2.5): LLM generates a targeted Cypher MATCH query (AST-validated read-only). **Corrective RAG gate** (v2.5): per-entity relevance score filters low-signal entities. **Episode hint** (v2.5): Sørensen–Dice retrieval of past similar tasks appended as routing context. Valkey cache TTL=1h |
