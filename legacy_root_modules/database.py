...
async def create_api_key(user_id: str, label: str = "") -> tuple[str, dict]:
    ...
    key_dict = {"id": kid, "user_id": user_id, "key_prefix": key_prefix,
                ...
                "last_used_at": None, "expires_at": None, "cc_profile_id": None, "dynamic_routing": False}
    return raw, key_dict
...
async def list_api_keys(user_id: str) -> list[dict]:
    async with _get_pool().connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT id,user_id,key_prefix,label,is_active,created_at,last_used_at,expires_at,cc_profile_id,dynamic_routing "
...
async def get_active_key_hashes(user_id: str) -> list[dict]:
    ...
    async with _get_pool().connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT id, key_hash, cc_profile_id, dynamic_routing FROM api_keys WHERE user_id=%s AND is_active=TRUE",
                (user_id,),
            )
            return await cur.fetchall()
